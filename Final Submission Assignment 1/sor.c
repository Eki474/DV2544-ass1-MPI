...
* Last Modified by : Rosen Sasov
*
* Parallel version
*****************************************************/
/* Initializing in the main function with calling Init_Matrix.
Passing and returning needed values to and from the function.
Passing those values to work() function call where most of the work will be
done.
Starting from Master.
At first iteration is not receiving data, every next receiving data
from worker with rank N-1. Master working.
Master sending to the worker with rank 1.
Worker with rank 1 receiving. Working. Sending to worker with rank 2.
Worker with rank 2 receiving. Working. Sending to worker with rank 3.
And so on..
---------------------------------------------------------------------
The worker with the last rank receiving. Working. Sending to master.
I think the problem is the finishing.
*/
...
main(int argc, char **argv)
{
  ...
  timestart= MPI_Wtime();
  printf("\n Main function: BEFORE work() functioncall");
  iter = work(rank,nprocT,rows_node);
  printf("\n Main function: AFTER work() functioncall");
  timeend = MPI_Wtime();
  //MPI_Finalize();
  ...
  while (!finished)
  {
    ...
    if(rank==0)
    {
      ...
      if(iteration>1)
      {
        ...
        mtype = FROM_WORKER;
        src = nproc - 1;
        printf("\n\tBefore Master Receive. Beginning of the work()\n");
        ...
        MPI_Recv(&glob->A[offset_rows][1], N, MPI_DOUBLE, src, mtype, MPI_COMM_WORLD, &status);
        printf("\n\tAfter Master Receive. Beginning of the work()\n");
      }
      offset_rows=1;
    ...
    {
      printf("\nInside while/ FIRST WORKER RECEIVE FROM MASTER\n");
      //FIRST WORKER RECEIVING DATA FROM MASTER
      mtype = FROM_MASTER;
      src = 0;
      printf("\n\tBefore First Worker Receive. Beginning of the work()\n");
      MPI_Recv(&offset_rows, 1, MPI_INT, src, mtype, MPI_COMM_WORLD, &status);
      MPI_Recv(&rows_node, 1, MPI_INT, src, mtype, MPI_COMM_WORLD, &status);
      MPI_Recv(&glob->A[offset_rows][1], N, MPI_DOUBLE, src, mtype, MPI_COMM_WORLD, &status);
      if(offset_rows< N-offset_rows) offset_rows+=rows_node;
      printf("\n\tAfter First Worker Receive. Beginning of the work()\n");
    }

    if(rank>1 && rank<nproc)
    {
      //WORKERS RECEIVING DATA FROM WORKERS
      mtype = FROM_WORKER;
      src = rank - 1;
      printf("\n\tBefore Worker Receive. Beginning of the work()\n");
      MPI_Recv(&offset_rows, 1, MPI_INT, src, mtype, MPI_COMM_WORLD, &status);
      MPI_Recv(&rows_node, 1, MPI_INT, src, mtype, MPI_COMM_WORLD, &status);
      MPI_Recv(&glob->A[offset_rows-1][0], N, MPI_DOUBLE, src, mtype, MPI_COMM_WORLD, &status);
      if(offset_rows< N-offset_rows) offset_rows+=rows_node;
      printf("\n\tAfter Worker Receive. Beginning of the work()\n");
    }
    ...
    if (turn == EVEN_TURN) {
      ...
      else if (turn == ODD_TURN)
      {
        ...
        // MASTER TASK
        if(rank==0 && rank+1!=nproc && !finished)
        { /*MASTER SHOULD NOT SEND DATA IN A MODE WITH 1 NODE AND
          WHEN THE FLAG FOR FINISH IS ACTIVE*/
          printf("\nInside Master Send Task at the end of work() \n Sending to workers...\n");
          mtype = FROM_MASTER;

          //MASTER SENDING TO WORKERS
            printf("\nBEFORE MASTER TASK SENDING \n Sending to workers...\n");
            MPI_Send(&offset_rows, 1, MPI_INT, 1, mtype, MPI_COMM_WORLD);
            MPI_Send(&rows_node,1,MPI_INT, 1,mtype, MPI_COMM_WORLD);
            MPI_Send(&glob->A[offset_rows][1], N, MPI_DOUBLE, 1, mtype, MPI_COMM_WORLD);
            printf("\nAFTER MASTER TASK SENDING\n Sending to worker...\n");
        }

        // WORKERS TASK
        if(rank>0 && rank<nproc-1)
        {
          // SENDING DATA FROM WORKER TO WORKER WITH NEXT RANK
          mtype = FROM_WORKER;
          dest = rank+1;
          MPI_Send(&offset_rows, 1, MPI_INT, dest, mtype, MPI_COMM_WORLD);
          MPI_Send(&rows_node, 1, MPI_INT, dest, mtype, MPI_COMM_WORLD);
          MPI_Send(&glob->A[offset_rows][1], N, MPI_DOUBLE, dest, mtype, MPI_COMM_WORLD);
        }

        //WORKER WITH LAST RANK SENDING DATA TO MASTER
        if(rank == nproc-1){
          dest = 0;
          mtype = FROM_WORKER;
          MPI_Send(&offset_rows, 1, MPI_INT, dest, mtype, MPI_COMM_WORLD);
